"""GUI launcher for auto_editor.py and summarizer.py"""
import os
import queue
import subprocess
import sys
...
def build_editor_tab(nb):
    frame = ttk.Frame(nb)
    nb.add(frame, text="  CapCut 자동 편집기  ")
    ...
    # 영상 파일
    ttk.Label(frame, text="영상 파일").grid(row=0, column=0, sticky="w", **pad)
    ...
    ttk.Button(frame, text="찾아보기",
               command=lambda: video_var.set(
                   filedialog.askopenfilename(
                       title="영상 파일 선택",
                       filetypes=[("동영상", "*.mp4 *.mov *.mkv *.avi *.webm"), ("전체", "*.*")]
                   ) or video_var.get()
               )).grid(row=0, column=2, padx=(0, 12), pady=4)

    # CapCut 드래프트 폴더
    ttk.Label(frame, text="CapCut 초안 폴더").grid(row=1, column=0, sticky="w", **pad)
    draft_folder_var = tk.StringVar()
    for candidate in [
        ...
        if os.path.isdir(candidate):
            draft_folder_var.set(candidate)
            break
    ttk.Entry(frame, textvariable=draft_folder_var, width=52).grid(
        row=1, column=1, sticky="ew", padx=(0, 4), pady=4)
    ttk.Button(frame, text="찾아보기",
               command=lambda: draft_folder_var.set(
                   filedialog.askdirectory(title="CapCut 초안 폴더 선택") or draft_folder_var.get()
               )).grid(row=1, column=2, padx=(0, 12), pady=4)

    # 옵션
    opt = ttk.LabelFrame(frame, text="옵션", padding=8)
    opt.grid(row=2, column=0, columnspan=3, sticky="ew", padx=12, pady=6)
    opt.columnconfigure(1, weight=1)
    opt.columnconfigure(3, weight=1)

    model_var = tk.StringVar(value="base")
    lang_var = tk.StringVar(value="ko")
    silence_db_var = tk.StringVar(value="-40")
    min_silence_var = tk.StringVar(value="400")
    padding_var = tk.StringVar(value="80")
    min_seg_var = tk.StringVar(value="200")
    name_var = tk.StringVar(value="")
    no_stutter_var = tk.BooleanVar(value=False)

    ttk.Label(opt, text="Whisper 모델").grid(row=0, column=0, sticky="w", padx=(8, 4), pady=3)
    ttk.Combobox(opt, textvariable=model_var,
                 values=["tiny", "base", "small", "medium", "large"],
                 width=8, state="readonly").grid(row=0, column=1, sticky="w", pady=3)
    ttk.Label(opt, text="언어").grid(row=0, column=2, sticky="w", padx=(16, 4), pady=3)
    ttk.Entry(opt, textvariable=lang_var, width=6).grid(row=0, column=3, sticky="w", pady=3)

    ttk.Label(opt, text="무음 기준 (dB)").grid(row=1, column=0, sticky="w", padx=(8, 4), pady=3)
    ttk.Entry(opt, textvariable=silence_db_var, width=6).grid(row=1, column=1, sticky="w", pady=3)
    ttk.Label(opt, text="최소 무음 (ms)").grid(row=1, column=2, sticky="w", padx=(16, 4), pady=3)
    ttk.Entry(opt, textvariable=min_silence_var, width=6).grid(row=1, column=3, sticky="w", pady=3)

    ttk.Label(opt, text="패딩 (ms)").grid(row=2, column=0, sticky="w", padx=(8, 4), pady=3)
    ttk.Entry(opt, textvariable=padding_var, width=6).grid(row=2, column=1, sticky="w", pady=3)
    ttk.Label(opt, text="최소 구간 (ms)").grid(row=2, column=2, sticky="w", padx=(16, 4), pady=3)
    ttk.Entry(opt, textvariable=min_seg_var, width=6).grid(row=2, column=3, sticky="w", pady=3)

    ttk.Label(opt, text="초안 이름 (선택)").grid(row=3, column=0, sticky="w", padx=(8, 4), pady=3)
    ttk.Entry(opt, textvariable=name_var, width=22).grid(row=3, column=1, sticky="w", pady=3)
    ttk.Checkbutton(opt, text="버벅거림 컷 비활성화 (--no-stutter)",
                    variable=no_stutter_var).grid(row=3, column=2, columnspan=2, sticky="w", pady=3)

    # 실행 버튼
    btn_label_var = tk.StringVar(value="실행")
    run_btn = ttk.Button(frame, textvariable=btn_label_var, style="Accent.TButton")
    run_btn.grid(row=3, column=0, columnspan=3, padx=12, pady=8, sticky="ew")

    # 로그 (row=4, weight=1 로 확장)
    log = make_log(frame)
    log.grid(row=4, column=0, columnspan=3, sticky="nsew", padx=12, pady=(0, 12))

    def on_run():
        video = video_var.get().strip()
        draft_folder = draft_folder_var.get().strip()
        if not video:
            messagebox.showwarning("입력 오류", "영상 파일을 선택하세요.")
            return
        if not draft_folder:
            messagebox.showwarning("입력 오류", "CapCut 초안 폴더를 선택하세요.")
            return

        cmd = [
            sys.executable, os.path.join(SCRIPT_DIR, "auto_editor.py"),
            video, draft_folder,
            "--model", model_var.get(),
            "--lang", lang_var.get(),
            "--silence-db", silence_db_var.get(),
            "--min-silence-ms", min_silence_var.get(),
            "--padding-ms", padding_var.get(),
            "--min-segment-ms", min_seg_var.get(),
        ]
        if name_var.get().strip():
            cmd += ["--name", name_var.get().strip()]
        if no_stutter_var.get():
            cmd += ["--no-stutter"]
        ...
        def done(ok):
            ...
            btn_label_var.set("실행")
            if ok:
                messagebox.showinfo("완료", "CapCut 초안이 생성되었습니다.\nCapCut을 열어 확인하세요.")
            else:
...
def build_summarizer_tab(nb):
    ...
    ttk.Entry(frame, textvariable=url_var, width=60).grid(
        row=0, column=1, columnspan=2, sticky="ew", padx=(0, 12), pady=4)

    # 출력 폴더
    ttk.Label(frame, text="출력 폴더").grid(row=1, column=0, sticky="w", **pad)
    ...
    def on_run():
        ...
        cmd = [
            ...
            "--model", model_var.get().split(" ")[0],  # "small (권장)" → "small"
            "--lang", lang_var.get(),
